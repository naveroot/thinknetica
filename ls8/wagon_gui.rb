module WagonGui
  ...
  def show_all_cargo_wagons(train)
    train.each_wagon_in_train do |wagon|
      puts "#{wagon.name} #{wagon.type}
занято:#{wagon.occupied_volume}
свободно:#{wagon.available_volume}"
    end
  end

  def show_all_passenger_wagons(train)
    train.each_wagon_in_train do |wagon|
      puts "#{wagon.name} #{wagon.type}
  занято:#{wagon.occupied_seats}
  свободно:#{wagon.available_seats}"
    end
  end

  def show_all_wagons(train)
    show_all_cargo_wagons(train) if train.is_a? CargoTrain
    show_all_passenger_wagons(train) if train.is_a? PassengerWagon
  end
end
